    ...
    m_legendComboBox(createLegendBox())
{
    ...
    themeLabel->setStyleSheet("background-color: white;");
    settingsLayout->addWidget(m_themeComboBox);

    QLabel *animationLabel = new QLabel("Animation:");
    animationLabel->setStyleSheet("background-color:white");
    settingsLayout->addWidget(animationLabel);

    QLabel *legendLabel = new QLabel("Legend:");
    legendLabel->setStyleSheet("background-color:white");
    settingsLayout->addWidget(m_animatedComboBox);
    settingsLayout->addWidget(legendLabel);
    ...
    for (int i(0); i < listCount; i++) {
        ...
        for (int j(0); j < valueCount; j++) {
            ...
            QPointF value((j + (qreal) rand() / (qreal) RAND_MAX) * ((qreal) m_valueMax / (qreal) valueCount),
                          yValue);
            QString label = "Food Item " + QString::number(i) + ":" + QString::number(j);
...
QComboBox *ThemeWidget::createThemeBox() const
{
    ...
    themeComboBox->setStyleSheet("background-color: white;");
...
QComboBox *ThemeWidget::createAnimationBox() const
{
   ...
   animationComboBox->setStyleSheet("background-color: white;");
...
QComboBox *ThemeWidget::createLegendBox() const
{
    ...
    legendComboBox->setStyleSheet("background-color: white;");
...
QChart *ThemeWidget::createAreaChart() const
{
    ...
    chart->setTitle("Food Items Area chart");

    // The lower series initialized to zero values
    QLineSeries *lowerSeries = 0;
    QString name("Food Item ");
    int nameIndex = 0;
...
QChart *ThemeWidget::createBarChart(int valueCount) const
{
    Q_UNUSED(valueCount);
    QChart *chart = new QChart();
    chart->setTitle("Food Items Bar chart");

    QStackedBarSeries *series = new QStackedBarSeries(chart);
    for (int i(0); i < m_dataTable.count(); i++) {
        QBarSet *set = new QBarSet("Food item " + QString::number(i));
...
{
    QChart *chart = new QChart();
    chart->setTitle("Food Items Line chart");
...
{
    QChart *chart = new QChart();
    chart->setTitle("Food Items Pie chart");
...
QChart *ThemeWidget::createSplineChart() const
{
    // spine chart
    QChart *chart = new QChart();
    chart->setTitle("Food Items Spline chart");
    QString name("Food item ");
    int nameIndex = 0;
    for (const DataList &list : m_dataTable) {
...
QChart *ThemeWidget::createScatterChart() const
{
    // scatter chart
    QChart *chart = new QChart();
    chart->setTitle("Food items Scatter chart");
    QString name("Food item ");
    int nameIndex = 0;
    for (const DataList &list : m_dataTable) {
